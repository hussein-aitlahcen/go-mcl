...
func InitMclHelper(curve int) {
	...
	if err != 0 {
		log.Panicf("ERR mclBn_init curve=%d", curve)
	}
}
...
// Returns n = 2^{pow}. Useful when working with roots of unity.
// TODO: could speed this up using squaring
...
func (g *G1) Mul(y *Fr) {
	C.mclBnG1_mul(g.getPointer(), g.getPointer(), y.getPointer())
}
...
func (g *G2) Mul(y *Fr) {
	C.mclBnG2_mul(g.getPointer(), g.getPointer(), y.getPointer())
}
